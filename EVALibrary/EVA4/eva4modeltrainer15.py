...
class Train:
  def __init__(self, model, dataloader, optimizer, stats, scheduler=None, criterion1=None, criterion2=None, L1lambda=0, tb=None):
    ...
    self.metrices = {"mask": [list() for i in range(6)], "depth": [list() for i in range(6)]}
    self.size = 0
  ...
  def run(self):
    ...
    for batch_idx, data in pbar:
      # get samples
      fgbg, mask, depth = data['fgbg'].to(self.model.device), data['mask'].to(
          self.model.device), data['depth'].to(self.model.device)
      ...
      if self.criterion1 is not None:
        ...
        m_ssim = torch.clamp(
            (1 - msssim(mask_pred, mask, normalize=True)) * 0.5, 0, 1)
      ...
      d_ssim = torch.clamp(
          1 - msssim(depth_pred, depth, normalize=True)*0.5, 0, 1)
      ...
      if (n+1) % 3000 == 0:  # Need to change this later
        grid = torchvision.utils.make_grid(
            mask_pred.detach().cpu(), nrow=8, normalize=False)
        self.tb.add_image('imagesmask', grid, n)
        grid = torchvision.utils.make_grid(
            depth_pred.detach().cpu(), nrow=8, normalize=False)
        self.tb.add_image('imagesdepth', grid, n)

        saveresults(fgbg.detach().cpu(), "./plots/fgbg" +
                    str(n+1)+".jpg", normalize=True)
        saveresults(mask.detach().cpu(), "./plots/orimask"+str(n+1)+".jpg")
        saveresults(depth.detach().cpu(), "./plots/oridepth"+str(n+1)+".jpg")
        saveresults(mask_pred.detach().cpu(),
                    "./plots/predmask"+str(n+1)+".jpg")
        saveresults(depth_pred.detach().cpu(),
                    "./plots/preddepth"+str(n+1)+".jpg")

      # Backpropagation
      loss.backward()
      self.optimizer.step()
      # Let's compute the metrices for every batch...
      ...
      for i in range(6):
        self.metrices["mask"][i].append(e1[i])
        self.metrices["depth"][i].append(e2[i])
    ...
    # print(*e1)
    # print(*e2)
    f = open(f"{self.stats.path}/{self.stats.model.name}train.txt", "a")
    mask_metrices = [i/self.size for i in self.metrices["mask"]]
    depth_metrices = [i/self.size for i in self.metrices["depth"]]
    f.write(f"Mask {' '.join(mask_metrices)}")
    f.write(f"Depth {' '.join(depth_metrices)}")
    f.close()
    print("metrices for training set : ")
    print("\tMask")
    print(f"\t\t\t{' '.join(mask_metrices)}")
    print("\tDepth")
    print(f"\t\t\t{' '.join(depth_metrices)}")
...
class Test:
  ...
  def run(self):
    ...
    with torch.no_grad():
        ...
        for batch_idx, data in enumerate(self.dataloader):
            ...
            if batch_idx == 0:
              ...
              mdinp = torch.cat(
                  [orimp[:8, :, :, :], mp[:8, :, :, :], oridp[:8, :, :, :], dp[:8, :, :, :]], dim=0)
              show(mdinp)
    ...
    # These are quantitative measures
    f = open(f"{self.stats.path}/{self.stats.model.name}test.txt", "a")
    mask_metrices = [i/self.size for i in self.metrices["mask"]]
    depth_metrices = [i/self.size for i in self.metrices["depth"]]
    f.write(f"Mask {' '.join(mask_metrices)}")
    f.write(f"Depth {' '.join(depth_metrices)}")
...
class ModelTrainer:
  def __init__(self, model, optimizer, train_loader, test_loader, statspath, scheduler=None, batch_scheduler=False, criterion1=None, criterion2=None, L1lambda=0):
    ...
    self.criterion1 = criterion1
    self.criterion2 = criterion2
    self.train = Train(model, train_loader, optimizer, self.stats, self.scheduler if self.batch_scheduler else None,
                       criterion1=criterion1, criterion2=criterion2, L1lambda=L1lambda, tb=self.tb)
    self.test = Test(model, test_loader, self.stats, self.scheduler,
                     criterion1=criterion1, criterion2=criterion2, tb=self.tb)
